...
1. 아쉽게도 UTF-8 인코딩외 다른 텍스트 파일의 지원이 불가능합니다...ㅠㅠ
2. notepad+ 또는 워드패드로 실행하여 UTF-8로 변환하면 사용가능합니다.
3. 일부 특수문자가 셀에 기록될 경우 워크시트 깨짐현상이 발생합니다.
4. 현재 파악된 지원되지 않는 특수문자는 =가 두번 이상들어간 경우입니다. 해당 문자열이 들어간 부분의 파싱은 피해주세요.
